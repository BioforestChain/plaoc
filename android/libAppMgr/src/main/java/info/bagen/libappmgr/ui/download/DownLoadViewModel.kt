...
class DownLoadViewModel(
  ...
  private suspend fun downLoadAndSave(url: String, saveFile: String) {
    repository.downLoadAndSave(url, saveFile, isStop = {// 用来控制是否停止下载
      ...
      it.fold(onSuccess = { file ->
        uiState.value.downloadAppInfo = ZipUtil.getAppInfoByPreDecompressTarGz(file.absolutePath)
  ...
  private suspend fun decompressFile() {
    // 解压文件
    val enableUnzip = ZipUtil.decompress(mDownLoadProgress.downloadFile, FilesUtil.getAppUnzipPath())
    if (enableUnzip) {
