...
class DCIMViewModel(private val repository: DCIMRepository = DCIMRepository()) : ViewModel() {
    ...
    viewModelScope.launch {
      when (action) {
        is DCIMIntent.UpdateSpinnerState -> {
          uiState.value.showSpinner.value = action.show ?: !uiState.value.showSpinner.value
        }
        ...
        is DCIMIntent.UpdatePreviewState -> {
          uiState.value.showPreview.value = action.show ?: !uiState.value.showPreview.value
        }
        is DCIMIntent.UpdateViewerBarState -> {
          uiState.value.showViewerBar.value = action.show ?: !uiState.value.showViewerBar.value
        }
        ...
        is DCIMIntent.SetCurrentDCIMInfo -> {
          if (!action.checkPreview || uiState.value.showPreview.value) {
            uiState.value.curDCIMInfo.value = action.dcimInfo ?: uiState.value.checkedList[0]
          } else {
        ...
        is DCIMIntent.SetCurrentDCIMSpinner -> {
          uiState.value.curDcimSpinner = action.dcimSpinner ?: uiState.value.dcimSpinnerList[0]
        }
  ...
  private fun overlayCheckList(dcimInfo: DCIMInfo) {
    uiState.value.curDCIMInfo.value.checked.value = !uiState.value.curDCIMInfo.value.checked.value
    uiState.value.curDCIMInfo.value.overlay.value = !uiState.value.curDCIMInfo.value.overlay.value
  }
