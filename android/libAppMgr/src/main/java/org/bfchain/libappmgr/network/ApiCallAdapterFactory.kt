...
class ApiResultCallAdapter(private val responseType: Type) : CallAdapter<Any, Call<Any>> {
            ...
            override fun onResponse(call: Call<Any>, response: Response<Any>) {
                ...
                if (response.isSuccessful) {
                    ...
                    if (body == null) {
                        callback.onResponse(
                            this@ApiResultCall,
                            Response.success(ApiResult.IsError(-1, "body is null"))
                        )
                    } else {
                        if (body is ApiResult.IsSuccess<*>) {
                            if (body.errorCode == 0) {
                            ...
                            callback.onResponse(
                                this@ApiResultCall,
                                Response.success(ApiResult.IsError(-1, "body not IsSeccess"))
                            )
                        }
                    }
                    ...
                    callback.onResponse(
                        this@ApiResultCall,
                        Response.success(ApiResult.IsError(-1, throwable.message()))
                    )
                }
            }
