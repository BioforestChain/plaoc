...
const Support = function Support2() {
  return {
...
function updateAutoHeight(speed) {
  ...
  if (swiper.params.slidesPerView !== "auto" && swiper.params.slidesPerView > 1) {
    if (swiper.params.centeredSlides) {
      swiper.visibleSlides.each((index2, slide2) => {
...
function updateClickedSlide(e) {
  ...
  if (slide2 && slideFound) {
    ...
    if (swiper.virtual && swiper.params.virtual.enabled) {
      ...
      swiper.clickedIndex = $(slide2).index();
    }
  } else {
...
function slideTo(index2 = 0, speed = this.params.speed, runCallbacks = true, internal) {
  const swiper = this;
...
function onTouchStart(event) {
  ...
  data2.touchStartTime = Utils.now();
  swiper.allowClick = true;
  swiper.updateSize();
  ...
  if (e.type !== "touchstart") {
    ...
    if (doc.activeElement && $(doc.activeElement).is(data2.formElements) && doc.activeElement !== $targetEl[0]) {
...
function onTouchMove(event) {
  ...
  if (data2.isTouchEvent && doc.activeElement) {
    if (e.target === doc.activeElement && $(e.target).is(data2.formElements)) {
      data2.isMoved = true;
      swiper.allowClick = false;
      return;
    }
...
const Browser = function Browser2() {
  function isSafari() {
...
const Scrollbar = {
  setTranslate() {
    ...
    if (!swiper.params.scrollbar.el || !swiper.scrollbar.el)
      return;
    const { scrollbar: scrollbar2, rtlTranslate: rtl, progress } = swiper;
    const {
      dragSize,
      trackSize,
      $dragEl,
  ...
  updateSize() {
    ...
    if (!swiper.params.scrollbar.el || !swiper.scrollbar.el)
      return;
    const { scrollbar: scrollbar2 } = swiper;
    const { $dragEl, $el } = scrollbar2;
  ...
  setDragPosition(e) {
    const swiper = this;
    const { scrollbar: scrollbar2, rtlTranslate: rtl } = swiper;
    const {
      $el,
      dragSize,
      trackSize,
  ...
  onDragStart(e) {
    const swiper = this;
    const params = swiper.params.scrollbar;
    const { scrollbar: scrollbar2, $wrapperEl } = swiper;
    const { $el, $dragEl } = scrollbar2;
  ...
  init() {
    ...
    if (!swiper.params.scrollbar.el)
      return;
    const { scrollbar: scrollbar2, $el: $swiperEl } = swiper;
    const params = swiper.params.scrollbar;
    let $el = $(params.el);
...
const Zoom = {
  ...
  onTouchMove(e) {
    const swiper = this;
    const zoom2 = swiper.zoom;
    const { gesture, image, velocity } = zoom2;
    if (!gesture.$imageEl || gesture.$imageEl.length === 0)
      return;
    swiper.allowClick = false;
  ...
  enable() {
    ...
    if (Support.gestures) {
      swiper.$wrapperEl.on("gesturestart", slideSelector, zoom2.onGestureStart, passiveListener);
      swiper.$wrapperEl.on("gesturechange", slideSelector, zoom2.onGestureChange, passiveListener);
      swiper.$wrapperEl.on("gestureend", slideSelector, zoom2.onGestureEnd, passiveListener);
  ...
  disable() {
    ...
    const slideSelector = `.${swiper.params.slideClass}`;
    if (Support.gestures) {
      swiper.$wrapperEl.off("gesturestart", slideSelector, zoom2.onGestureStart, passiveListener);
      swiper.$wrapperEl.off("gesturechange", slideSelector, zoom2.onGestureChange, passiveListener);
      swiper.$wrapperEl.off("gestureend", slideSelector, zoom2.onGestureEnd, passiveListener);
    } else if (swiper.touchEvents.start === "touchstart") {
...
var zoom = {
  ...
  create() {
    ...
    "onGestureStart onGestureChange onGestureEnd onTouchStart onTouchMove onTouchEnd onTransitionEnd toggle enable disable in out".split(" ").forEach((methodName) => {
      zoom2[methodName] = Zoom[methodName].bind(swiper);
    });
    Utils.extend(swiper, {
      zoom: zoom2
    });
