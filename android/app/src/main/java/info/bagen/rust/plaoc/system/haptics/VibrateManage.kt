...
enum class HapticsNotificationType(
  ...
  SUCCESS(
    "SUCCESS", longArrayOf(0, 35, 65, 21), intArrayOf(0, 250, 0, 180), longArrayOf(0, 35, 65, 21)
  ),
  WARNING(
    "WARNING",
  ...
  ERROR("ERROR", longArrayOf(0, 27, 45, 50), intArrayOf(0, 120, 0, 25), longArrayOf(0, 27, 45, 50)),
}
...
class VibrateManage private constructor() {
  ...
  init {
    mVibrate = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
      val vm = App.appContext.getSystemService(Context.VIBRATOR_MANAGER_SERVICE) as VibratorManager
      vm.defaultVibrator
    } else {
  ...
  fun vibrate(duration: Long) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
      mVibrate.vibrate(VibrationEffect.createOneShot(duration, VibrationEffect.DEFAULT_AMPLITUDE))
    } else {
