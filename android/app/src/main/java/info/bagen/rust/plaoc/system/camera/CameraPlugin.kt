...
class CameraPlugin() {
    ...
    if (settings.saveToGallery && (imageEditedFileSavePath != null || imageFileSavePath != null)) {
      ...
      try {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
          ...
          val stream =
            resolver.openOutputStream(uri) ?: throw IOException("Failed to open output stream.")
          val inserted = bitmap.compress(Bitmap.CompressFormat.JPEG, settings.quality, stream)
          if (!inserted) {
            isSaved = false
          }
        } else {
  ...
  private fun returnDataUrl(
    exif: ExifWrapper, bitmapOutputStream: ByteArrayOutputStream, onCallback: ((String) -> Unit)?
  ): JSONObject {
  ...
  private fun createEditIntent(origPhotoUri: Uri?): Intent? {
    return try {
      ...
      val resInfoList: List<ResolveInfo> = App.appContext.packageManager.queryIntentActivities(
        editIntent,
