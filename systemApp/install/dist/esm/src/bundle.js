 ...
 * @returns {boolean}
 */
async function createBfsaDir(bfsAppId) {
 ...
 * @returns
 */
async function getBfsaMetaDataJson(bootPath) {
 ...
 * @returns
 */
async function copyIcon(bootPath, iconName) {
 ...
 * @returns
 */
async function writeConfigJson(bootPath, bfsAppId, metadata) {
    // bfsa-metadata.json
    await writeFile(path.join(bootPath, "bfs-metadata.json"), JSON.stringify(metadata), "utf-8");
    // 复制icon到boot目录
    ...
    const filesList = await fileListHash(destPath, bfsAppId, []);
    // link.json
    const linkJson = await genLinkJson(bfsAppId, metadata, filesList);
    await writeFile(path.join(bootPath, "link.json"), JSON.stringify(linkJson), "utf-8");
    return;
}
 ...
 * @returns
 */
function genLinkJson(bfsAppId, metadata, filesList) {
    const { manifest } = metadata;
    const iconName = path.basename(manifest.icon);
    const linkJson = {
 ...
 * @returns
 */
async function searchFile(src, nameReg) {
    const entries = await readdir(src, { withFileTypes: true });
    for (const entry of entries) {
 ...
 * @returns
 */
async function fileListHash(dest, bfsAppId, filesList) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isFile()) {
            ...
            const file = {
                url: `https://shop.plaoc.com/${bfsAppId}${filePath.slice(filePath.lastIndexOf(bfsAppId) + bfsAppId.length)}`,
                size: fileStat.size,
                sha512: fileHash,
            };
            filesList.push(file);
