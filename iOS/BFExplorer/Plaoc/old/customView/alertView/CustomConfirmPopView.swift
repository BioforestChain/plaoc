...
class CustomConfirmPopView: CustomBasePopView {
    ...
    var confirmModel: ConfirmConfiguration? {
        didSet {
            ...
            dismissButton.setTitle(confirmModel?.cancelText, for: .normal)
    ...
    override init(frame: CGRect) {
        ...
        containView.addSubview(dismissButton)
    ...
    override func addConstraint() {
        ...
        verlineView.snp.makeConstraints { make in
            make.top.equalTo(self.lineView.snp.bottom)
            make.left.equalTo(self.dismissButton.snp.right).offset(-0.5)
            make.width.equalTo(0.5)
            make.height.equalTo(40)
        }
        
        confirmButton.snp.remakeConstraints { make in
            make.left.equalTo(self.dismissButton.snp.right)
            make.right.equalTo(0)
            make.top.equalTo(self.dismissButton)
            make.height.equalTo(40)
        }
    }
