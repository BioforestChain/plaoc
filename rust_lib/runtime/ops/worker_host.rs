...
#[derive(Clone)]
pub struct PreloadModuleCbHolder(Arc<WorkerEventCb>);
...
fn op_create_worker(state: &mut OpState, args: CreateWorkerArgs) -> Result<WorkerId, AnyError> {
    ...
    if let WebWorkerType::Classic = worker_type {
        if let TestingFeaturesEnabled(false) = state.borrow() {
            return Err(deno_webstorage::DomExceptionNotSupportedError::new(
...
fn close_channel(state: Rc<RefCell<OpState>>, id: WorkerId, channel: WorkerChannel) {
