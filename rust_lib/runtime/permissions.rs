...
impl PermissionState {
    ...
    fn check(self, name: &str, info: Option<&str>, prompt: bool) -> (Result<(), AnyError>, bool) {
        match self {
...
impl UnitPermission {
    ...
    pub fn request(&mut self) -> PermissionState {
        if self.state == PermissionState::Prompt {
            if permission_prompt(&format!("access to {}", self.description), self.name) {
...
impl UnaryPermission<ReadDescriptor> {
    ...
    pub fn check_blind(&mut self, path: &Path, display: &str) -> Result<(), AnyError> {
    ...
    pub fn check_all(&mut self) -> Result<(), AnyError> {
        let (result, prompted) = self.query(None).check(self.name, Some("all"), self.prompt);
        if prompted {
            if result.is_ok() {
                self.global_state = PermissionState::Granted;
            } else {
...
impl UnaryPermission<NetDescriptor> {
    ...
    pub fn request<T: AsRef<str>>(&mut self, host: Option<&(T, Option<u16>)>) -> PermissionState {
        if let Some(host) = host {
            ...
            if state == PermissionState::Prompt {
                if permission_prompt(&format!("network access to \"{}\"", host), self.name) {
    ...
    pub fn check<T: AsRef<str>>(&mut self, host: &(T, Option<u16>)) -> Result<(), AnyError> {
...
impl UnaryPermission<EnvDescriptor> {
    ...
    pub fn check(&mut self, env: &str) -> Result<(), AnyError> {
        let (result, prompted) =
            self.query(Some(env))
                .check(self.name, Some(&format!("\"{}\"", env)), self.prompt);
        if prompted {
            if result.is_ok() {
                self.granted_list.insert(EnvDescriptor::new(env));
            } else {
...
impl UnaryPermission<FfiDescriptor> {
    ...
    pub fn check(&mut self, path: Option<&Path>) -> Result<(), AnyError> {
        if let Some(path) = path {
            ...
            let (result, prompted) = self.query(None).check(self.name, None, self.prompt);
...
impl deno_net::NetPermissions for Permissions {
    fn check_net<T: AsRef<str>>(&mut self, host: &(T, Option<u16>)) -> Result<(), AnyError> {
        self.net.check(host)
    }
    ...
    fn clear_stdin() -> Result<(), AnyError> {
        ...
        unsafe fn emulate_enter_key_press(stdin: HANDLE) -> Result<(), AnyError> {
            ...
            *input_record.Event.KeyEvent_mut().uChar.UnicodeChar_mut() = '\r' as WCHAR;
            ...
            let success = WriteConsoleInputW(stdin, &input_record, 1, &mut record_written);
            if success != TRUE {
                bail!(
        ...
        unsafe fn is_input_buffer_empty(stdin: HANDLE) -> Result<bool, AnyError> {
            ...
            let success = PeekConsoleInputW(stdin, buffer.as_mut_ptr(), 1, &mut events_read);
            if success != TRUE {
                bail!(
...
#[cfg(test)]
static PERMISSION_PROMPT_STUB_VALUE_SETTER: Lazy<Mutex<PermissionPromptStubValueSetter>> =
...
mod tests {
    ...
    fn check_specifiers() {
        ...
        if cfg!(target_os = "windows") {
            fixtures.push((resolve_url_or_path("file:///C:/a/mod.ts").unwrap(), true));
            fixtures.push((resolve_url_or_path("file:///C:/b/mod.ts").unwrap(), false));
        } else {
    ...
    fn test_query() {
        ...
        let perms2 = Permissions {
            read: UnaryPermission {
                ...
                ..Permissions::new_read(&Some(vec![PathBuf::from("/foo")]), false).unwrap()
            },
            write: UnaryPermission {
                global_state: PermissionState::Prompt,
                ..Permissions::new_write(&Some(vec![PathBuf::from("/foo")]), false).unwrap()
            },
            net: UnaryPermission {
                global_state: PermissionState::Prompt,
            ...
            ffi: UnaryPermission {
                global_state: PermissionState::Prompt,
                ..Permissions::new_ffi(&Some(vec![PathBuf::from("deno")]), false).unwrap()
            },
    ...
    fn test_revoke() {
        let mut perms = Permissions {
            ...
            net: UnaryPermission {
                global_state: PermissionState::Prompt,
                ..Permissions::new_net(&Some(svec!["127.0.0.1", "127.0.0.1:8000"]), false).unwrap()
            },
            env: UnaryPermission {
                global_state: PermissionState::Prompt,
                ..Permissions::new_env(&Some(svec!["HOME"]), false).unwrap()
