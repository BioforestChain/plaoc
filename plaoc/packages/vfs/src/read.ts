 ...
 * @returns 
 */
export async function rename(path: string, newName: string): Promise<string | boolean> {
  // 提取文件前缀 /a/b/bfsa.txt -> /a/b/
  const newPath = Path.join(path.slice(0, path.lastIndexOf("/") + 1), newName)
  const fs = await network.asyncCallDenoFunction(vfsHandle.FileSystemRename, { path, newPath })
  if (fs === "true") {
    return true
  }
  return fs
